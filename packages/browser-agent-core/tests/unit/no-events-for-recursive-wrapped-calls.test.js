...
import {createWrapperWithEmitter} from '../../common/wrap/wrap-function'
...
test('recursive calls to wrapped functions from start/end event callbacks should not trigger more events', done => {
...
test('calls to other wrapped functions from start/end event callbacks should not trigger more events', done => {
  let ee = baseEE.get(eeId++)
...
test('calls to other wrapped functions from start/end event callbacks with different prefix should not trigger more events', done => {
  let ee = baseEE.get(eeId++)
  let wrapper = createWrapperWithEmitter(ee)
...
test('always flag allows nested calls', done => {
  let fooCallCount = 0
  let barCallCount = 0
  let fooStartCount = 0
  let barStartCount = 0
