...
import { getRuntime, setInfo, setConfiguration } from '../../common/config/config'
import { Aggregate as AjaxAggreg } from '../../features/ajax/aggregate/index'
...
test('storeXhr for a SPA ajax request buffers in spaAjaxEvents', done => {
...
test('storeXhr for a non-SPA ajax request buffers in ajaxEvents', done => {
...
test('on interactionDiscarded, saved SPA events are buffered in ajaxEvents', done => {
  const interaction = { id: 0 }
  const context = {
    spaNode: { interaction: interaction }
  }
...
test('prepareHarvest correctly serializes an AjaxRequest events payload', done => {
  const context = { spaNode: undefined }
  const expected = {
    type: 'ajax',
    start: 0,
...
test('prepareHarvest correctly serializes a very large AjaxRequest events payload', done => {
  const context = { spaNode: undefined }
  let callNo = 0
  const expected = () => ({
    type: 'ajax',
...
test('prepareHarvest correctly exits if maxPayload is too small', done => {
  const context = { spaNode: undefined }
  let callNo = 0
  const expected = () => ({
    type: 'ajax',
