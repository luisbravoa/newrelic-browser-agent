...
import { getRuntime, setRuntime, setInfo } from '../../common/config/config'
import { submitData } from '../../common/util/submit-data'
import * as harvest from '../../common/harvest/harvest'
import * as sinon from 'sinon'
import * as encode from '../../common/url/encode'
import * as locationUtil  from '../../common/url/location'
...
test('returns false if nothing is sent', done => {
  resetSpies()
  setupFakeNr();
...
test('encodes only the origin of the referrer url, not the fragment ', done => {
  resetSpies()
...
test('encodes referrer urls that include spaces', done => {
...
test('encodes referrer urls that include ampersands', done => {
...
test('uses correct submission mechanism for ins', done => {
  if (xhrUsable) {
    t.plan(4)
  } else {
    t.plan(2)
...
test('does not send ins call when there is no body', done => {
  resetSpies()
...
test('uses correct submission mechanism for resources', done => {
  if (xhrUsable) {
    t.plan(4)
  } else {
    t.plan(2)
...
test('does not send resources when there is no body', done => {
  resetSpies()
...
test('uses an XHR and returns it for first resources POST', done => {
  resetSpies()
  harvesterInst.on('resources', once(dummyPayload('resources')))
  setupFakeNr();
...
test('uses correct submission mechanism for jserrors', done => {
  if (xhrUsable) {
    t.plan(4)
  } else {
    t.plan(3)
...
test('adds ptid to harvest when ptid is present', done => {
  if (xhrUsable) {
    t.plan(2)
  } else {
    t.plan(2)
...
test('does not add ptid to harvest when ptid is not present', done => {
  if (xhrUsable) {
    t.plan(2)
  } else {
    t.plan(2)
...
test('does not send jserrors when there is nothing to send', done => {
  resetSpies()
...
test('uses correct submission mechanism for events', done => {
  if (xhrUsable) {
    t.plan(4)
  } else {
    t.plan(3)
...
test('does not send eents when there is nothing to send', done => {
  resetSpies()
...
test('uses correct submission mechanisms on unload', done => {
  resetSpies()
...
describe('when sendBeacon returns false', done => {
  test('uses img for jserrors', done => {
    let baseUrl = scheme + '://foo/jserrors/1/bar?a=undefined&v='+ VERSION +'&t=Unnamed%20Transaction&rst={TIMESTAMP}&ref='+ scheme + '://foo.com&q1=v1&q2=v2'
    let expectedPayload = {jserrors: ['one', 'two', 'three']}
  ...
  test('uses img for resources', done => {
    let baseUrl = scheme + '://foo/resources/1/bar?a=undefined&v='+ VERSION +'&t=Unnamed%20Transaction&rst={TIMESTAMP}&ref='+ scheme + '://foo.com&q1=v1&q2=v2'
    let expectedPayload = {resources: ['one', 'two', 'three']}
...
describe('response codes', () => {
  ...
  function runTest(type, responseCode, testCase) {
    test('returns correct result with ' + responseCode, done => {
      t.plan(1)
