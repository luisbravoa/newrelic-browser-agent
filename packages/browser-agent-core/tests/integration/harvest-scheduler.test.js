...
import { setRuntime, setInfo } from '../../common/config/config'
import * as sinon from 'sinon'
import * as harv from '../../common/harvest/harvest'
...
test('after calling startTimer, periodically invokes harvest', done => {
  resetSpies()
  var calls = 0
...
test('scheduleHarvest invokes harvest once', done => {
  resetSpies()
...
test('when getPayload is provided, calls harvest.send', done => {
  resetSpies()
  var scheduler = new HarvestScheduler('endpoint', { onFinished: onFinished, getPayload: getPayload }, aggregator.sharedContext)
  scheduler.startTimer(0.1)
...
test('when getPayload is not provided, calls harvest.sendX', done => {
  resetSpies()
...
test('does not call harvest.send when payload is null', done => {
  resetSpies()
  var scheduler = new HarvestScheduler('endpoint', { getPayload: getPayload }, aggregator.sharedContext)
  scheduler.startTimer(0.1)
...
test('provides retry to getPayload when submit method is xhr', done => {
...
test('when retrying, uses delay provided by harvest response', done => {
  resetSpies({
