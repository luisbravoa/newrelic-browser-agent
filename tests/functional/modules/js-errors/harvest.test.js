...
testDriver.test('jserrors are retried when collector returns 429', es6, function (t, browser, router) {
  let assetURL = router.assetURL('modular/send-scoped-noticeerror-error-obj.html', {
    init: {
      jserrors: {
  ...
  Promise.all([errPromise, loadPromise]).then(([errResult]) => {
